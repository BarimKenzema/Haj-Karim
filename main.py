...
print("--- GITHUB COLLECTOR v45.1 (Flow Detection + Premium Filtering + FIXED) START ---")
...
PREMIUM_FILE = 'premium_configs.txt'  # NEW: Best configs (Reality + flow)
MAX_ACTIVE_CONFIGS = 4444
MAX_PREMIUM_CONFIGS = 500  # NEW: Keep best 500 premium configs
MAX_DB_SIZE_MB = 44
...
def sanitize_filename(name):
    ...
    # Windows: < > : " / \ | ? *
    # Also remove & and other URL characters that might cause issues
    ...
    # Limit length (filesystems typically support 255, but be conservative)
    if len(name) > 100:
    ...
    return name.lower()  # Make it lowercase for consistency

# =========================
# NEW: Flow & Quality Detection
# =========================

def extract_flow_setting(config_str):
    """
    Extract the 'flow' setting from a config.
    Returns: flow value string or None
    """
    try:
        if config_str.startswith('vmess://'):
            # VMess doesn't have flow
            return None
...
def calculate_config_quality_score(config_str):
    ...
    Higher score = better for bypassing censorship.
    
    Scoring:
    - flow=xtls-rprx-vision: +50
    - security=reality: +40
    - security=tls: +20
    - protocol=vless: +15
    - network=tcp: +10
    - network=grpc: +8
    - has valid port (443, 8443): +5
    """
    score = 0
...
def is_premium_config(config_str):
    ...
    Premium = Reality security + xtls-rprx-vision flow
    """
    try:
        flow = extract_flow_setting(config_str)
        attrs = get_config_attributes(config_str)
        ...
        # Premium criteria:
        # 1. Must be VLESS (only protocol that supports flow)
        # 2. Must have Reality or TLS security
        # 3. Must have xtls-rprx-vision flow
        
        has_good_flow = flow and 'xtls-rprx-vision' in flow
        has_good_security = security in ['reality', 'tls', 'xtls']
        is_vless = protocol == 'vless'
        
        # Strictest: Reality + vision flow
        if is_vless and security == 'reality' and has_good_flow:
            return True
        
        # Also good: TLS + vision flow
...
def get_config_attributes(config_str):
    ...
    try:
        ...
        return {
            ...
            'security': security, 
            'country': country,
            'flow': flow  # NEW: Include flow in attributes
        }
    except Exception:
        return None
...
def setup_directories():
    import shutil
    dirs = ['./splitted', './subscribe', './protocols', './networks', './countries', './security', './flow']
    for d in dirs:
        if os.path.exists(d):
            shutil.rmtree(d)
        os.makedirs(d)
...
def write_chunked_subscription_files(base_filepath, configs):
    ...
    """
    # Sanitize the base filepath to prevent directory traversal issues
...
def fetch_from_github_code_search():
    ...
    queries = [
        ...
        # NEW: Search specifically for xtls-rprx-vision configs
...
def main():
    ...
    # === NEW: PREMIUM CONFIG FILE ===
    print(f"\n{'='*70}")
    print(f"  PREMIUM CONFIGS (Best for Iran)")
    print(f"{'='*70}")
    
    if premium_configs:
        # Process premium configs (convert to IP, rename)
        premium_processed = []
        ...
        with open('premium_plain.txt', 'w', encoding='utf-8') as f:
            f.write('\n'.join(merged_premium[:100]))  # Top 100 as plain text
    ...
    for item in processed:
        ...
        # NEW: Categorize by flow (with proper sanitization)
        flow = attrs.get('flow')
        if flow:
            flow_key = sanitize_filename(flow)  # FIXED: Use sanitize_filename
    ...
    # NEW: Save flow-based categories (with sanitization)
    try:
