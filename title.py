    ...
    for element in array_configuration:
        try:
            ...
            if protocol_type == "VMESS":
                ...
                config_dict = {k.lower(): v for k, v in config_dict.items()} # Normalize keys
                
                host, port, config_id = config_dict.get('add'), int(config_dict.get('port', 0)), config_dict.get('id')
                params = config_dict # For VMESS, the whole dict is params
                def final_config_builder(title, ip):
                    config_dict['ps'] = title
                    config_dict['add'] = ip
                    return f"vmess://{base64.b64encode(json.dumps(config_dict).encode('utf-8')).decode('utf-8')}"
            
            else: # For all other link-based protocols
                parsed_url = urlparse(element)
                host, port, config_id = parsed_url.hostname, parsed_url.port, parsed_url.username
                params = parse_qs(parsed_url.query)
                def final_config_builder(title, ip):
                    # For other protocols, we can't just change the host to an IP, so we keep original
            ...
            final_config = final_config_builder(title, ip_address)
            ...
            # print(f"--> [SKIP] Error processing config. Original: {element[:70]}")
            continue
